...
import com.example.TomTomIntegration.rest.swagger.DuplicateExceptionSwaggerExample;
import com.example.TomTomIntegration.rest.swagger.PoiNotFoundExceptionSwaggerExample;
...
public class PoiCrudController {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Successfully created"),
            @ApiResponse(responseCode = "404", description = "Poi with name 'Restaurant Bavaria' already exists", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = DuplicateExceptionSwaggerExample.class))
            })})
    @PostMapping
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Successfully received", content = {
            ...
            @ApiResponse(responseCode = "404", description = "Poi by id 1 was not found", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = PoiNotFoundExceptionSwaggerExample.class))
            })})
    @GetMapping("/{poiId}")
